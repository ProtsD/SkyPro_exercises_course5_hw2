...
import ru.skypro.skypro_exercises_course5_hw1.dto.ReportDTO;
...
public interface EmployeeRepository extends CrudRepository<Employee, Integer>, PagingAndSortingRepository<Employee, Integer> {
    ...
    @Query("SELECT new ru.skypro.skypro_exercises_course5_hw1.dto.EmployeeFullInfo(e.name, e.salary, p.name) " +
    ...
    @Query("SELECT new ru.skypro.skypro_exercises_course5_hw1.dto.ReportDTO(" +
            "p.name, " +
            "COUNT(e.name), " +
            "MAX(e.salary), " +
            "MIN(e.salary), " +
