...
import ru.skypro.skypro_exercises_course4_hw7.dto.ReportDTO;
...
public interface EmployeeRepository extends CrudRepository<Employee, Integer>, PagingAndSortingRepository<Employee, Integer> {
    ...
    @Query("SELECT new ru.skypro.skypro_exercises_course4_hw7.dto.EmployeeFullInfo(e.name, e.salary, p.name) " +
    ...
    @Query("SELECT new ru.skypro.skypro_exercises_course4_hw7.dto.ReportDTO(" +
            "p.name, " +
            "COUNT(e.name), " +
            "MAX(e.salary), " +
            "MIN(e.salary), " +
